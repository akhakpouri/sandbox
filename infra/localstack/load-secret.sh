...
kubectl create secret generic vault-bootrap-secret -n $NAMESPACE \
  --from-literal=VAULT_TOKEN="$VAULT_TOKEN" \
  --from-literal=VAULT_POD="$VAULT_POD" \
  --from-literal=AUTH_TOKEN="$AUTH_TOKEN" \
  --from-literal=SECRET_PATH="$SECRET_PATH"
